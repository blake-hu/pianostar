...
uint16_t samples[BUFFER_SIZE] = {
    0}; // stores ADC samples and PWM duty cycle values
...
static void saadc_event_callback(nrfx_saadc_evt_t const *event) {
  if (event->type == NRFX_SAADC_EVT_DONE) {
    ...
    for (int i = 0; i < BUFFER_SIZE; i++) {
      ...
      samples[i] =
          (((int32_t)samples[i] - average) * 10) + (ADC_MAX_COUNTS / 2);
    }
...
static void adc_init(void) {
  ...
  //    acquisition time
  nrf_saadc_channel_config_t mic_channel_config =
      NRFX_SAADC_DEFAULT_CHANNEL_CONFIG_SE(ANALOG_MIC_IN);
